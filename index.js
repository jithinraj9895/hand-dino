...
(function () {
    ...
    function DistanceMeter(canvas, spritePos, canvasWidth) {
        this.canvas = canvas;
        this.canvasCtx = canvas.getContext('2d'); // <-- FIXED
...
function onDocumentLoad() {
    ...
    // MediaPipe Hands setup for fist gesture jump
    // Create a hidden video element for webcam
...
function isFist(landmarks) {
    // Simple heuristic: all fingertips are close to their respective MCP joints
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        if (isPinch(landmarks)) {
            ...
            if (now - lastFistTime > 300) { // Debounce: 600ms between jumps
                lastFistTime = now;
                // Trigger jump
                if (window.Runner && Runner.instance_ && !Runner.instance_.crashed) {
